...
export async function maybeCreateDirectorNote(context) {
  ...
  if (roll > threshold) {
    debugDirector("Skipped", { reason: "frequency", frequency, threshold, roll });
    return;
  }

  const creatureType = context.creatureType ?? "humanoid";
  const library = DIRECTOR_NOTES[creatureType] ?? DIRECTOR_NOTES.humanoid;
  const usedCreatureType = DIRECTOR_NOTES[creatureType] ? creatureType : "humanoid";

  const note = pickNarration(library, context, `director:${usedCreatureType}`);

  if (!note) {
    debugDirector("Skipped", { reason: "no-note", creatureType, usedCreatureType });
    return;
  }

  const outputMode = getSetting("directorOutputMode") ?? "gmBlind";
  const content = renderDirectorNote({ note, context, frequency, outputMode });
  ...
  debugDirector("Generated", {
    ...
    roll,
    outputMode,
    selected: { id: note.id, key: note.key, text: note.text }
  });
